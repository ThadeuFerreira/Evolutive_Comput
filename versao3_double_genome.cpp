/*
*   Sexuada
*   A cada geração os dois primeiros colocados na geração aterior trocam
*   os dois cromossomos e geram filhos com uma chance 
*
*/
...
int main()
{
    ...
    for(int gen = 0; gen < GENERATIONS; gen++)
    {
        ...
        for(int i = 2; i < POPULATION; i++)
        {
            for(int j = 0; j < GENES; j++)
            {
                Competidor[i].Genotipo_1[j] = genesis(Competidor[0].Genotipo_1[j], MUTATIONRATE);
				Competidor[i].Genotipo_2[j] = genesis(Competidor[1].Genotipo_2[j], MUTATIONRATE);
            }
            featinessCalc(&Competidor[i]);
        }
